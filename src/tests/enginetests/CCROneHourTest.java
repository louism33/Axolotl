...
class CCROneHourTest {
    ...
    void ccrTests() {
        ...
        String[] splitUpCCRs = ccrTests.split("\\\n");
        int totalKaufs = splitUpCCRs.length;

        for (int ccr = 0; ccr < totalKaufs; ccr++){

            System.out.println("Now checking CCR test number "+ (ccr + 1));

            ExtendedPositionDescriptionParser.EDPObject edpObject = ExtendedPositionDescriptionParser.parseEDPPosition(splitUpCCRs[ccr]);
...
//            Assert.assertEquals(destination, edpObject.getBestMoveDestinationIndex());
            ...
            if (destination == edpObject.getBestMoveDestinationIndex()){
                System.out.println("SUCCEEDED CCR test number "+ (ccr + 1));
                successes++;
            }
            else {
                System.out.println("FAILED CCR test number "+ (ccr + 1));
                failures++;
            }
        ...
        System.out.println();
        System.out.println("-----");
        System.out.println("Total tests: " + splitUpCCRs.length);
