...
public class QuickTest {
    ...
    void QuickTest() {
        ...
        String[] splitUpQuickTests = quickTests.split("\\\n");
        int totalKaufs = splitUpQuickTests.length;

        for (int quick = 0; quick < totalKaufs; quick++){

            System.out.println("Now checking QuickTest test number "+ (quick + 1));

            ExtendedPositionDescriptionParser.EDPObject edpObject = ExtendedPositionDescriptionParser.parseEDPPosition(splitUpQuickTests[quick]);
...
//            Assert.assertEquals(destination, edpObject.getBestMoveDestinationIndex());
            ...
            if (destination == edpObject.getBestMoveDestinationIndex()){
                System.out.println("SUCCEEDED QuickTest test number "+ (quick + 1));
                successes++;
            }
            else {
                System.out.println("FAILED QuickTest test number "+ (quick + 1));
                failures++;
            }
        ...
        System.out.println();
        System.out.println("-----");
        System.out.println("Total tests: " + splitUpQuickTests.length);
