...
public class KaufmanTests {
    ...
    void kaufmanTests() {
        ...
        String[] splitUpKaufmans = kaufmanTests.split("\\\n");
        int totalKaufs = splitUpKaufmans.length;
        
        for (int kauf = 0; kauf < totalKaufs; kauf++){

            System.out.println("Now checking Kaufman test number "+ (kauf + 1));

            ExtendedPositionDescriptionParser.EDPObject edpObject = ExtendedPositionDescriptionParser.parseEDPPosition(splitUpKaufmans[kauf]);
...
//            Assert.assertEquals(destinationIndex, edpObject.getBestMoveDestinationIndex());
            ...
            if (destination == edpObject.getBestMoveDestinationIndex()){
                ...
                System.out.println("FAILED Kaufman test number "+ (kauf + 1));
                failures++;
            }
