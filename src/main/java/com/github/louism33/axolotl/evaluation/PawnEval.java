...
public final class PawnEval {
    ...
    static final int[] pawnScores = new int[2];
    ...
    static long[] calculatePawnData(Chessboard board) {
        ...
        for (int turn = WHITE; turn <= BLACK; turn++) {
            ...
            for (int f = 0; f < 8; f++) {
                final int fnp = populationCount(FILES[f] & ps);
                if (fnp > 1) {
                    myPawnScore -= (fnp - 1) * PAWN_DOUBLED;
                }
            }

            long protectedPawns = allPawnCaptures & ps;
            while (protectedPawns != 0) {
                myPawnScore += ((7 * turn) - (numberOfTrailingZeros(protectedPawns) / 8)) * PAWN_PROTECTED_BY_PAWNS;
                protectedPawns &= protectedPawns - 1;
            }

            long neighbourPawns = allAdjacentSquares & ps;
            while (neighbourPawns != 0) {
                myPawnScore += ((7 * turn) - (numberOfTrailingZeros(neighbourPawns) / 8)) * PAWN_NEIGHBOURS;
                neighbourPawns &= neighbourPawns - 1;
            }

            pawnScores[turn] = myPawnScore;
        }
