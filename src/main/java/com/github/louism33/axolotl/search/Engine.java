...
//        System.out.println("*******************************");
    ...
    static int principleVariationSearch(int depth, int ply,
        ...
        if (ply == 0 && hashMove != 0) {
            Assert.assertEquals(moves[0] & MOVE_MASK, hashMove);
        }
        ...
        for (int i = 0; i < lastMove; i++) {
            ...
            if (!thisIsAPrincipleVariationNode) {
                ...
                if (false && isFutilityPruningAllowedHere(depth,
            ...
            if (alpha >= beta) {
                ...
                updateHistoryMoves(whichThread, move, ply);
                break;
            }
        }
        ...
        addToTableReplaceByDepth(board.zobristHash,
                bestMove & MOVE_MASK, bestScore, depth, flag, ply);
